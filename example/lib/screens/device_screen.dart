...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  void initState() {
    ...
    Timer.periodic(Duration(milliseconds: 2000), (timer) async {
      ...
      if (mounted && isConnected && (_services.length > 0)) {
        debugPrint(_services.toString());

        for (BluetoothService s in _services) {
          debugPrint('HHD: service: ${s.uuid}');
          if (s.uuid.toString() == pilot_service_uuid) {
            pilotService = s;
            try {
              _characteristics = await pilotService!.characteristics;
            } catch (e) {
              debugPrint('HHD: error: ${e.toString()}');
            }
          }
  ...
  Widget buildPilotStatusTile(BuildContext context, BluetoothService? service) {
    if (service == null) {
      return Container();
    }
    return EvsePilotStatusTile(service: service);
  }
  //added by hhd end
